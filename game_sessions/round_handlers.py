...
class FlowerFruitVegRoundHandler(BaseRoundHandler):
    ...
    def create_player_answer(self, player, answer_text: str):
        ...
        # Return unsaved object for bulk creation - this fixes race conditions
        return PlayerAnswer(
            player=player,
            round_number=self.round_number,
    ...
    def validate_answer_manually(self, player_answer, is_valid: bool) -> int:
        ...
        # Always create a ScoreHistory entry for manual validation tracking
        # This is crucial for detecting that an answer has been manually processed
        if points_difference != 0:
            ...
            # Even if no points change (e.g., invalid answer with 0 points),
            # create a ScoreHistory entry to track that manual validation occurred
            from players.models import ScoreHistory
            ScoreHistory.objects.create(
                player=player_answer.player,
                game_session=player_answer.player.game_session,
                round_number=self.round_number,
                points_change=0,
                reason=reason,
                related_answer=player_answer
...
class MultipleChoiceRoundHandler(BaseRoundHandler):
    ...
    def _generate_new_question(self):
        ...
        from .ai_questions import get_question_for_game_with_exclusions
        ...
        # Get list of questions already used in this game session
        used_question_ids = list(self.game_session.used_questions.values_list('id', flat=True))

        question_obj = get_question_for_game_with_exclusions(
            category=category,
    ...
    def _get_fallback_question_data(self):
        ...
        # First try to get questions not used in this game
        unused_questions = list(MultipleChoiceQuestion.objects.exclude(id__in=used_question_ids))
        if unused_questions:
            question = random.choice(unused_questions)
            logger.info(f"Using fallback database question (unused in this game): {question.question_text[:50]}...")
            # Add to used questions to prevent future repeats
            self.game_session.used_questions.add(question)
            return {
                'question_text': question.question_text,
                'choices': question.choices,
        ...
        # If all questions have been used in this game, use any question
        all_questions = list(MultipleChoiceQuestion.objects.all())
        if all_questions:
            question = random.choice(all_questions)
            logger.warning(f"All questions used in this game! Reusing: {question.question_text[:50]}...")
            return {
                'question_text': question.question_text,
                'choices': question.choices,
                'correct_answer': question.correct_answer,
...
ROUND_HANDLERS = {
    ...
    # mastermind handler now handled in dedicated app
}
...
def get_round_handler(game_session, round_number: int, round_type: str = None) -> BaseRoundHandler:
    ...
    # Special handling for mastermind rounds - use dedicated app
    if round_type == 'mastermind':
        from mastermind.handlers import MastermindRoundHandler
        return MastermindRoundHandler(game_session, round_number)
    
    # Get handler class for other round types
